...
const accountSid = 'ACd1dc353a85bc13fedcb5b4aa01df7716'
const authToken = '743e483c8a96aa16bcbe66cd5624ecf9'
const myPhoneNumber = '+59176279020';

const client = twilio(accountSid, authToken);
//------conexion a slack 
const webHook=new IncomingWebhook("https://hooks.slack.com/services/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")
...
const loggerStream={
  write:message=>{
    ...
    console.log("mensaje de alerta enviado!")
  }
}
//funcion para enviar por slack 
function enviarAlertaslack(datosActuales) {
  ...
  ☔ Nivel de agua Río La Paz: Elevado
  ...
  .then(() => console.log("Mensaje de alerta enviado a Slack"))
  .catch(error => console.error("Error al enviar mensaje a Slack:", error));
}
/*
...
const umbrales = {
  ...
  humedad: 10, // Porcentaje
  pluviometro: 25, // Milímetros
  camara1: 10000
};
...
app.post('/rec', (req, res) => {
  console.log("peticion post recibirda")
  const data = req.body;
  console.log("Datos recibidos:", data);

  // Aquí puedes añadir código para hacer algo con los datos recibidos
  // Por ejemplo, guardarlos en una base de datos, etc.

  res.send('Datos recibidos correctamente');
});
//-----------------------DIRECCION PARA MULTER------------
...
function publicarEnMQTT(topico, mensaje) {
  mqttClient.publish(topico, mensaje, (err) => {
    if (err) {
      console.error('Error al publicar en MQTT:', err);
   
} else {
  console.log('Mensaje publicado correctamente en', topico);
}
});
}
  ...
  mqttClient.on("message", async (topic, message) => {
    ...
    try {
      ...
      // Si hay un error en el parseo, simplemente imprime un mensaje y retorna
      console.error('Error al parsear el mensaje:', error);
      return; // No procesa más este mensaje
    }
...
//------------------------------SOBRE EL MENSAJES DE ALERTA----------------------

// Comprobar si se superan los umbrales
if (puedeEnviarAlerta) {
  let mensajeAlerta = '';
  let hayAlertaCritica = false;

  // Priorizar alerta de cámara
  if (datosActuales.camara1 > umbrales.camara1) {
    mensajeAlerta = 'Alerta crítica de cámara. ';
    hayAlertaCritica = true;
  }

  // Comprobar otros sensores
  if (datosActuales.temperatura > umbrales.temperatura) {
    mensajeAlerta += 'Temperatura alta. ';
  }
  if (datosActuales.humedad > umbrales.humedad) {
    mensajeAlerta += 'Humedad alta. ';
  }
  if (datosActuales.pluviometro > umbrales.pluviometro) {
    mensajeAlerta += 'Pluviometría alta. ';
  }

  // Enviar alerta si es necesario
  if (mensajeAlerta) {
    puedeEnviarAlerta = false; // Deshabilitar nuevas alertas
    const topic2 = `monitoreo/alarma`;
    const data = hayAlertaCritica ? 2 : 1; // 2 para crítica, 1 para normal
    const message2 = data.toString();

    mqttClient.publish(topic2, message2, (err) => {
      if (err) {
        console.error('Error al publicar en MQTT:', err);
      } else {
        console.log('Mensaje publicado correctamente en', topic2);
        //sendSMS(mensajeAlerta); // Enviar SMS con el mensaje de alerta
        enviarAlertaslack(datosActuales);
      }
    });

    console.log("Publicado en monitoreo/alarma:", mensajeAlerta);

    // Re-habilitar el envío de alertas después de un tiempo adaptativo
    setTimeout(() => {
      puedeEnviarAlerta = true;
    }, hayAlertaCritica ? 120000 : 60000); // Tiempo más largo para alertas críticas
  }
}
...
const csvDownloadUrl = ' https://8365-190-181-24-101.ngrok-free.app/archivo.csv'; // Reemplaza con la URL real
...
function formatDate(date) {
  ...
  // Configurar la zona horaria a La Paz, Bolivia (UTC-4)
  d.tz('America/La_Paz');
  ...
  try {
    ...
    for (const topic of topics) {
      ...
      data.forEach(item => {
        ...
        if (!organizedData[formattedTimestamp]) {
          organizedData[formattedTimestamp] = { timestamp: formattedTimestamp };
        }
        if (!organizedData[formattedTimestamp][topic]) {
          organizedData[formattedTimestamp][topic] = [];
        }
        organizedData[formattedTimestamp][topic].push(item.value);
      });
    }
...
async function getLatestDataByTopic(topic, limit) {
  try {
    ...
    const collection = db.collection('datos');
    let data = await collection.find({ topic }).sort({ timestamp: -1 }).limit(limit).toArray();

    // Formatear cada timestamp a la zona horaria de Bolivia
    data = data.map(item => ({
      ...item,
